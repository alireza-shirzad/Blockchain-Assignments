...
contract EBookLibrary is CustomERC20{
  ...
  function TypeAccount() public view returns (AccountType)
  {
      return accoutTypes[msg.sender];
  }
     ...
     */
    function addbook(uint bookId,BookGenre bookGenre,uint accessFee) 
    onlyAdmin
    public{
    Book memory newbook;
    newbook.isValid = true;
    newbook.accessFee = accessFee;
    newbook.bookGenre = bookGenre;
    books[bookId] = newbook;
    
    
    rentInfo memory newrentinfo;
    newrentinfo.renter = msg.sender;
    newrentinfo.startTime = now;
    newrentinfo.available = true;
    rentedBooks[bookId] = newrentinfo;
    }
    ...
    public 
    {
    require(balanceOf(msg.sender)>=premiumFee);
    transfer(admin,premiumFee);
     ...
     * @param bookId ID of the book
     */
    function accesss(uint bookId)
    onlyIfbookExists(bookId) 
    public{
    if(accoutTypes[msg.sender]==AccountType.premium && accoutTypes[rentedBooks[bookId].renter]==AccountType.regular){
        rentedBooks[bookId].available = true;
        transferFrom(admin,rentedBooks[bookId].renter,books[bookId].accessFee);
    }
    require(rentedBooks[bookId].available==true);
    require(balanceOf(msg.sender)>=books[bookId].accessFee);
